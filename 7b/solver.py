...
def canFindEquation(equationTotal: int, nums: list) -> bool:
    ...
    if equationTotal < nums[0]:
        print("--total too large, returning False--")
        return False
    if len(nums) == 1:
        if equationTotal == nums[0]:
    ...
    return canFindEquation(equationTotal, [nums[0] * nums[1]] + nums[2:]) or canFindEquation(equationTotal, [nums[0] + nums[1]] + nums[2:])
...
for equation in rawInputs:
    ...
    if canFindEquation(equationTotal, nums):
